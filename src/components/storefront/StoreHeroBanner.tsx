      ...
      <div className="relative grid gap-8 px-6 py-10 lg:grid-cols-[minmax(0,1.2fr)_340px] lg:px-10 lg:py-14">
        <div>
          ...
          <h2 className="mt-5 max-w-3xl text-3xl font-bold leading-tight lg:text-5xl">
            {store.heroTitle || <>Shop smarter from <span className="text-pink-300">{store.name}</span> with a premium online experience.</>}
          </h2>
          <p className="mt-4 max-w-2xl text-base leading-7 text-slate-300 lg:text-lg">
            {store.heroSubtitle || "Discover featured products, trusted Cash on Delivery checkout, and a polished shopping journey designed for fast-growing stores across Pakistan."}
          </p>
            ...
            {[
              ...
              return (
                <span key={label} className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2">
                  <ItemIcon className="h-4 w-4 text-pink-300" /> {label}
                </span>
              );
            })}
          </div>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <a href={`https://wa.me/${store.whatsapp.replace(/\D/g, "")}`} className="btn-secondary border-white/20 bg-white/10 text-white hover:bg-white/15">
              Ask on WhatsApp
            </a>
          </div>
        </div>
